...
for file in os.listdir(reads_folder): #For every file in folder containing reads
    if file.endswith(file_end): #Check if file is a .fastq-read-file
        ...
        current_list = list([write_folder + "/" + sample + "_outFwd_1.fastq.gz", 
        ...
        if reverse_flag == "yes":
            subprocess.run([f'cd {reads_folder} ; cutadapt -j 0 --max-n=0 --untrimmed-output {current_list[6]} --untrimmed-paired-output {current_list[7]} -g {regular_primer} -G {reverse_primer} -o {current_list[2]} -p {current_list[3]} {file.replace(repl_from, repl_to)} {file}'], shell=True) #Run cutadapt in reverse direction
